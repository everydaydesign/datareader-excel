...
type ZipCtx = { bytes: Uint8Array; dec: TextDecoder; view: DataView };
...
/** True for an OLE compound-document header (0xD0CF11E0) — an encrypted / legacy .xls workbook. */
...
/** Method 0 (stored) passes through; method 8 (deflate) inflates under `budget`; else reject. */
async function decompress(method: number, comp: Uint8Array, budget: number): Promise<Uint8Array> {
  if (method === 0) return comp;
  if (method === 8) return inflateRaw(comp, budget);
  throw new XlsxError(`unsupported ZIP compression method ${method}`);
}

/** Read one central-directory entry at `p`: validate the record, resolve its local-header data range,
 * decompress it under `budget`, and report the entry name plus the next directory offset. */
async function readEntry(
  ctx: ZipCtx,
  p: number,
  budget: number,
): Promise<{ data: Uint8Array; name: string; nextP: number }> {
  const { bytes, dec, view } = ctx;
  ...
  const data = await decompress(method, comp, budget);
  return { data, name, nextP: p + 46 + nameLen + extraLen + commentLen };
}
...
export async function unzip(
  ...
  if (isEncrypted(bytes)) throw new XlsxError("encrypted workbooks are unsupported");
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  const eocd = findEocd(view);
  const cdCount = view.getUint16(eocd + 10, true);
  let p = view.getUint32(eocd + 16, true); // central directory offset
  const dec = new TextDecoder("utf-8");
  const ctx: ZipCtx = { bytes, dec, view };
  const out = new Map<string, Uint8Array>();
  let budget = maxInflatedBytes;
  for (let i = 0; i < cdCount; i++) {
    const entry = await readEntry(ctx, p, budget);
    budget -= entry.data.byteLength;
    if (budget < 0) throw new XlsxError("inflated output exceeds the byte budget");
    if (!entry.name.endsWith("/")) out.set(entry.name, entry.data);
    p = entry.nextP;
  }
  return out;
}
